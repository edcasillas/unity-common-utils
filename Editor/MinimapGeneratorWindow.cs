...
namespace CommonUtils.Editor {
	public class MinimapGeneratorWindow : EditorWindow {
		...
		private void OnGUI() {
			...
			var tmpMask = EditorGUILayout.MaskField(new GUIContent("Culling Mask", "Culling mask to be used by the camera that will take the picture of the map."),  InternalEditorUtility.LayerMaskToConcatenatedLayersMask(cullingMask), InternalEditorUtility.layers);
			cullingMask = InternalEditorUtility.ConcatenatedLayersMaskToLayerMask(tmpMask);
			//EditorExtensions.ReadOnlyLabelField("Culling mask value", cullingMask.value);
